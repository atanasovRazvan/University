...
public class NotaXMLService extends AbstractService<String, Nota> {

    public NotaXMLService(NotaXMLRepository repo){
        super(repo);
    }
    ...
    public int getNotaCurenta(String idNota, Tema tema, int intarziere){
        ...
        if(intarziere==-1) return super.findOne(idNota).get_grade();
        int deadline = tema.getDeadline();
        int delay = super.findOne(idNota).getSaptPredata() - deadline;
        if(delay>2) return 1;
        if(delay>0) {
            super.findOne(idNota).setPredatLaTimp(false);
